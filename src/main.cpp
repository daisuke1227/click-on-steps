...
// getTimestampCompat is used for custom keybind compatibility.
// Since iOS relies solely on touch, we simply return 0 on iOS.
std::uint64_t getTimestampCompat() {
#ifdef GEODE_IS_IOS
    return 0;
#else
    // For non-iOS platforms (e.g. macOS), if keyboard support were enabled,
    // this code would be used. (Currently, keyboard code is removed.)
    return 0;
#endif
}
...
void CustomGJBaseGameLayer::queueButton(int btnType, bool push, bool secondPlayer) {
#endif
    // This is a workaround for passing arguments.
    auto& fields = this->m_fields;

#ifdef GEODE_IS_ANDROID
    // On Android, if the player is currently in a frame, directly insert the input.
    if (fields->m_inFrame) {
    ...
    if (inputTimestamp < timeRelativeBegin || !inputTimestamp || !timeRelativeBegin) {
        // If time isn't initialized, set to 0.
        currentTime = 0;
    }

    // Calculate the input offset; this preserves compatibility with physics bypass.
    ...
    // If the offset would result in a negative time, reset to zero.
...
void CustomGJBaseGameLayer::processTimedInputs() {
    // Calculate the current time offset (in ms) to determine input handling.
    auto& fields = m_fields;
...
void CustomGJBaseGameLayer::updateInputQueue() {
    // Failsafe: adjust input times when cutoff is disabled.
    auto& fields = this->m_fields;
    auto& commands = fields->m_timedCommands;
...
void CustomGJBaseGameLayer::dumpInputQueue() {
    // Failsafe: force-process unhandled inputs.
    auto& fields = this->m_fields;
    auto& commands = fields->m_timedCommands;
...
#ifdef GEODE_IS_WINDOWS
    // Windows workaround for inlined queueButton logic.
